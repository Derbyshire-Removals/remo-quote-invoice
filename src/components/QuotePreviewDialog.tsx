    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-[210mm] w-full max-h-[85vh] p-8 bg-white overflow-y-auto">
        <div className="w-full relative">
          ...
          <div className="customer-address">{document.fromAddress}</div>

          <div className="greeting">Dear {document.customerName},</div>

          <div className="quote-message">{document.message || 'No message provided'}</div>

          <div className="items-list">
            {document.items?.map((item: any) => (
              <div className="item" key={item.id}>
                <span className="item-description">{item.description}</span>
                <span className="item-amount">£{parseFloat(item.amount).toFixed(2)} + VAT</span>
              </div>
            )) || ''}
          </div>
